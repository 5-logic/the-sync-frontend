...
export default function ChecklistEditPage() {
	...
	const detectChanges = useCallback(() => {
		...
		const originalItemsMap = new Map(
			originalItems.filter((item) => item.id).map((item) => [item.id, item]),
		);
		const existingItemIds = new Set(
			existingItems.filter((item) => item.id).map((item) => item.id),
		);
	...
	const handleChangeField = useCallback(
		(id: string, field: keyof ChecklistItem, value: string | boolean) => {
			...
			setChecklistItems((prev) =>
				prev.map((item) =>
					item.id && item.id === id ? { ...item, [field]: value } : item,
				),
			);
		},
		[isSaving],
	...
	const handleConfirmedDelete = useCallback(
		async (itemId: string) => {
			try {
				...
				setChecklistItems((prev) =>
					prev.filter((i) => i.id && i.id !== itemId),
				);
	...
	const handleDeleteItem = useCallback(
		async (item: ChecklistItem) => {
			...
			if (isTemporaryItem) {
				...
				setChecklistItems((prev) =>
					prev.filter((i) => i.id && i.id !== item.id),
				);
