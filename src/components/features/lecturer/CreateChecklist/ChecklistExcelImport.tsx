...
function validateLength(
	...
	if (actualLength < rules.minLength) {
		errors.push(
			`Row ${rowNumber}: ${fieldName} must be at least ${rules.minLength} characters (current: ${actualLength} characters: "${value}")`,
		);
	}
	if (actualLength > rules.maxLength) {
		errors.push(
			`Row ${rowNumber}: ${fieldName} must be less than ${rules.maxLength} characters (current: ${actualLength} characters)`,
		);
	}
	return errors;
}
